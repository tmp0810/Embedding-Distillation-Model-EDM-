...
def finetune(
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in train_dataloader:
            st_time = time.time()
            # Giả định batch là một tuple (input_batch, output_batch, _)
            input_batch, output_batch, _ = batch
            dataset["train"].move_to_device([input_batch, output_batch], device)

            # Tính loss_denom cho từng batch (có thể cần điều chỉnh theo yêu cầu cụ thể)
            loss_denom = output_batch["label"].ne(-100).sum() / args.batch_size

            # Đoạn code chính 1: Huấn luyện mô hình
            ...
            # Đoạn code chính 2: Cập nhật các biến theo dõi
